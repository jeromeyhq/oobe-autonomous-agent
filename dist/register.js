    ...
    return [
        {
            ...
            price_per_call: new anchor_1.BN(1000000),
        ...
        },
        {
            ...
            price_per_call: new anchor_1.BN(2000000),
            min_price_per_call: null,
            max_price_per_call: null,
            rate_limit: 50,
        ...
        },
        {
            ...
            price_per_call: new anchor_1.BN(1500000),
            min_price_per_call: null,
            max_price_per_call: null,
            rate_limit: 80,
...
function loadKeypair(keypairPath) {
    ...
    if (fs.existsSync(keypairPath)) {
        ...
        return web3_js_1.Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(keypairPath, 'utf-8'))));
    }
...
async function registerAgent() {
    ...
    try {
        const client = (0, synapse_sap_sdk_1.createSapClient)(rpcUrl, wallet);
        console.log('✅ SapClient initialized');
        // Derive PDAs using real SDK functions
        const [agentPda] = synapse_sap_sdk_1.Pdas.getAgentPDA(keypair.publicKey);
        const [agentStatsPda] = synapse_sap_sdk_1.Pdas.getAgentStatsPDA(agentPda);
        const [globalPda] = synapse_sap_sdk_1.Pdas.getGlobalPDA();
...
async function dryRunRegistration() {
    const keypairPath = process.env.WALLET_KEYPAIR_PATH || './keys/agent-keypair.json';
    const keypair = loadKeypair(keypairPath);
    const [agentPda] = synapse_sap_sdk_1.Pdas.getAgentPDA(keypair.publicKey);
    ...
    // Validate manifest
    try {
        synapse_sap_sdk_1.Utils.validateAgentInput({
            name: AGENT_NAME,
